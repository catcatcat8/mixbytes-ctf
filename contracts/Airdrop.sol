...
contract Airdrop {
    ...
    function withdraw(bytes32[] memory merkleProof) external {
        ...
        require(Merkle.proofHash(merkleProof) == merkleRoot, 'Merkle proof failed');
    ...
    function nextDrop() public view returns (uint256) {
        return dropPerAddress < token.balanceOf(address(this)) ? dropPerAddress : token.balanceOf(address(this));
    }
...
library Merkle {
    ...
    function makeTree(address[8] memory addressList)
        internal
        pure
        returns (
            bytes32[] memory tree,
